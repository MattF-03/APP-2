import random   # On importe la bibliothèque random permettant de générer des nombres ou de faire des choix au hasard
...
def identifiant(i):
    return "{}{}".format(PREFIXES[i % len(PREFIXES)], 100 + (i * 7) % 900) 
        # On veut trouver un identifiant périodique dans la liste PREFIXES
        # On génère un numéro entre 100 et 999 qui change à chaque avion afin d'avoir un identifiant

def avion_info(i, carburant, medical, technique_prob, diplomatique_niv):
    return {
        "id": identifiant(i),   # Identifiant généré 
        "fuel": carburant,   # Carburant passé en paramètre
        "medical": medical,   # Urgence militaire 
        "technical_issue": technique_prob,   # Urgence technique
        "diplomatic_level": diplomatique_niv,   # Niveau d'importance diplomatique du vol   
        "arrival_time": round(19.40 + i * 0.02, 3)   # On calcule l'heure d'arrivée prévue de chaque avion  
    }
...
def generer_normal(nombre, valeur_initiale = 42):
    ...
    for i in range(nombre):
        ...
        technique_prob = random.random() < 0.05   # 5% de chance d'incident technique
        diplomatique_niv = random.randint(1, 3)   # Niveau d'importance diplomatique bas (1 à 3) 

        avions.append(avion_info(i, carburant, medical, technique_prob, diplomatique_niv))   # On ajoute le dictionnaire retourné par avion_info à la liste avions
...
def generer_crise_carburant(nombre, valeur_initiale = 1):
    ...
    for i in range(nombre):
        ...
        technique_prob = random.random() < 0.05   
        diplomatique_niv = random.randint(1, 3)   

        avions.append(avion_info(i, carburant, medical, technique_prob, diplomatique_niv)) 
...
def generer_chaos(nombre, valeur_initiale = 99):
    ...
    for i in range(nombre):
        ...
        technique_prob = random.random() < 0.15   # 15% des avions ont un incident technique
        diplomatique_niv = random.randint(1, 5)   # Tous les niveaux possibles

        avion = avion_info(i, carburant, medical, technique_prob, diplomatique_niv)
        avion["id"] = "CH{:03d}".format(i)   
        # On écrase l'identifiant généré par identifiant() pour le remplacer par un identifiant afin d'identifier facilement que cet avion vient du scénario chaos
...
GENERATEURS = {
    ...
    "carburant_crise": generer_crise_carburant,
    "medical_crise": generer_crise_medicale,
    "diplomatique_sommet": generer_sommet_diplomatique,
    "chaos": generer_chaos,
}
...
def afficher_stats(avions, nom):
    ...
    for i in avions:
        if i["diplomatic_level"] >= 4:
            diplo += 1

    total_carburant = 0   # On calcule le carburant moyen de tous les avions
    for i in avions:
        total_carburant = total_carburant + i["fuel"]
    moy = total_carburant / len(avions)

    print(f"--- RÉSULTATS : {nom} ---")   # On affiche les statistiques ligne par ligne
    print()
    print(f"  Nombre total d'avions : {len(avions)}")
    print(f"  Problèmes médicaux    : {med}")
    print(f"  Problèmes techniques  : {tech}")
