...
# On définit une fonction de simulation prenant une liste d'avions, une policy choisie pour les atterrissages, un nom pour la simulation et l'affichage affiché par défaut
    ...
    while len(file) > 0:   # On répète la boucle tant qu'il reste des avions en attente
        ...
        for i in file:   # On parcourt chaque avion de la file
            i["carburant"] -= 1   # On diminue le carburant de chaque avion de 1
        ...
        for j in file:   # On parcourt chaque avion pour les séparer selon leur carburant
            if j["carburant"] <= 0:   # Si l'avion n'a plus de carburant, alors il se crashe
        ...
        for k in morts:  # On parcourt chaque avion crashé
            ...
            if afficher:  
                print(f"  Tour {tour} | CRASH : {i['index']}")   # On affiche le numéro du tour et l'identifiant de l'avion crashé
        ...
        if afficher:   
            print(f"  Tour {tour} | OK : {atterri['index']} Carburant = {atterri['carburant']} En attente = {len(file)}")  # On affiche les informations de l'atterrissage

    taux = len(sauves) / len(avions) * 100   # On calcule le pourcentage d'avions sauvés
    print(f"  --- {nom} | Sauves = {len(sauves)} Crashes = {len(crashes)} Taux = {taux:.1f}%")   # On affiche le bilan de la simulation
...
def comparer_policies(avions, policies):   # On définit une fonction qui compare plusieurs policies d'atterrissage
    ...
    for nom, p in policies.items():   # On parcourt chaque policy du dictionnaire
        b = simuler(avions, p, afficher=False)   # On simule les atterrissages sans affichage
        b["nom"] = nom   # On ajoute le nom de la policy au bilan
        bilans.append(b)   # On ajoute le bilan à la liste des bilans

    bilans.sort(key = lambda b: len(b["crashes"]))   # On trie les bilans par nombre de crashes croissant

    print("\nPolicy  Sauves  Crashes  Taux")   # On affiche l'en-tête du tableau de comparaison
    for b in bilans:   # On parcourt chaque bilan
        print(f"{b['nom']}  {len(b['sauves'])}  {len(b['crashes'])}  {b['taux']:.1f}%")   # On affiche les résultats de chaque policy
